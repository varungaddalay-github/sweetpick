#!/usr/bin/env python3
"""
Safe migration script with efficient chunked processing to avoid gRPC limits.
...
class SafeHybridMigrationV3:
    """Safe migration with efficient chunked processing."""
    ...
    async def migrate_existing_data_chunked(self) -> bool:
        """Migrate existing data in small chunks to avoid gRPC limits."""
        
        print("\n🔄 Migrating Existing Data (Chunked)")
        print("="*40)
        ...
        try:
            ...
            # Process in small chunks to avoid gRPC limits
            chunk_size = 50  # Small chunks to avoid message size limits
            total_migrated = 0
            ...
            for offset in range(0, total_dishes, chunk_size):
                ...
                # Prepare chunk data with hybrid defaults
                migrated_chunk = []
                ...
                # Insert chunk into new collection
    ...
    async def run_migration(self) -> bool:
        ...
        print("🚀 Starting Safe Hybrid Migration V3 (Chunked)")
        print("="*45)
        ...
        print(f"   Method: Create new collection and migrate data (chunked)")
        
        # Step 2: Create new collection with hybrid schema
        if not await self.create_new_collection_with_hybrid_schema():
            return False
        
        # Step 3: Migrate existing data in chunks
        if not await self.migrate_existing_data_chunked():
            return False
...
async def main():
    ...
    migrator = SafeHybridMigrationV3()
